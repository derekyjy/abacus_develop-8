...
代码上仍然保持旧接口不变，上层仍通过 `Grid_Driver::init()` 和 `Grid_Driver::Find_atom()` 使用邻接信息。

后续建议：

1. 给 `GridParallel` 增加正式 MPI 测试，验证多 rank 下邻居表和串行结果一致。
2. 在 runner 的内存统计里补上 `box_bounds`，当前报告里的内存数据沿用旧统计口径，主要用于时间对比。
3. 如果要进入生产路径，可以在 `Grid_Driver` 或 `atom_arrange::search()` 层面增加 MPI/串行路径选择开关。
